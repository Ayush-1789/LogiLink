        ...
        detailedRoute: [
          {
            ...
            duration: '6h 15m',
            distance: '320 km',
            borderCrossing: shipmentDetails.origin.includes('Germany') ? 'Germany-Netherlands' : null
          },
          {
...
// New Air-only options to ensure we have fastest and cost-effective for air filter
{
  id: 13,
  modes: ['Air'],
  cost: 1950.50, // Cheaper air option to be cost-effective when filtering for air only
  transitTime: 3.5,
  co2Emissions: 'High',
  reliability: 'High',
  borderCrossings: 1,
...
// New Sea-only options to ensure we have fastest and cost-effective for sea filter
{
  id: 15,
  modes: ['Sea'],
  cost: 950.50, // Cheapest sea option
  transitTime: 26,
  co2Emissions: 'Low',
  reliability: 'Medium',
  borderCrossings: 0,
...
// New Land-only options to ensure we have fastest and cost-effective for land filter
{
  id: 17,
  modes: ['Land'],
  cost: 1250.30, // Cheaper land option
  transitTime: 16,
  co2Emissions: 'Medium',
  reliability: 'High',
  borderCrossings: 3,
...
},
{
  ...
  const getFilteredRoutes = () => {
    if (activeFilter === 'all') {
      ...
      return routeOptions.filter(route => 
        route.modes.length === 1 && route.modes.includes('Air')
      );
    } else if (activeFilter === 'sea') {
      return routeOptions.filter(route => 
        route.modes.length === 1 && route.modes.includes('Sea')
      );
    } else if (activeFilter === 'land') {
      return routeOptions.filter(route => 
        route.modes.length === 1 && route.modes.includes('Land')
      );
    } else {
      return routeOptions;
    }
  ...
  const getFeaturedRoutes = () => {
    ...
    const fastest = filteredRoutes.reduce((fastestRoute, currentRoute) => {
      if (!fastestRoute || currentRoute.transitTime < fastestRoute.transitTime) {
        return currentRoute;
      }
      return fastestRoute;
    }, null);
    ...
    const costEffective = filteredRoutes.reduce((cheapestRoute, currentRoute) => {
    ...
    <div className="shipping-app">
      ...
      {!showResults ? (
        <div className="form-container">
          <form onSubmit={handleSubmit}>
              ...
              <div className="form-section">
                ...
                <div className="form-row">
                  <div className="form-field full-width checkbox-container">
                    <label className="checkbox-group-label">Special Requirements</label>
            ...
            <div className="form-actions">
              <button type="submit" className="primary-button">Find Optimal Routes</button>
              <button 
                type="button" 
        ...
        <div className="detailed-route-container">
          ...
          <div className="route-overview">
            <p className="route-summary">
              From <strong>{shipmentDetails.origin}</strong> to <strong>{shipmentDetails.destination}</strong>
            </p>
            <div className="overview-cards">
              <div className="overview-card">
                ...
                <div className="overview-details">
                  ...
                  <span className="overview-value">${selectedRoute.cost.toFixed(2)}</span>
                </div>
              </div>
              <div className="overview-card">
                ...
                <div className="overview-details">
                  ...
                  <span className="overview-value">{selectedRoute.transitTime} days</span>
                </div>
              </div>
              <div className="overview-card">
                ...
                <div className="overview-details">
                  ...
                  <span className="overview-value">
                    {calculateRouteStats(selectedRoute.detailedRoute).totalDistance} km
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="route-timeline">
            {selectedRoute.detailedRoute.map((segment, index) => (
              <div key={index} className="timeline-segment">
                <div className={`timeline-point mode-${segment.mode.toLowerCase()}-point`}>
                  <div className="timeline-point-icon"></div>
                </div>
                <div className="timeline-line">
                  {index < selectedRoute.detailedRoute.length - 1 && (
                    <div className={`timeline-mode-line mode-${segment.mode.toLowerCase()}-line`}></div>
                  )}
                </div>
                <div className="timeline-card">
                  <div className="timeline-header">
                    <h3 className={`timeline-title mode-${segment.mode.toLowerCase()}-text`}>
                    ...
                    {segment.mode === 'Air' && segment.flightDetails && (
                      <div className="additional-details">
                        ...
                        <div className="detail-item">
                          ...
                          <span className="detail-value">{segment.flightDetails.flightNumber}</span>
                        </div>
                        <div className="detail-item">
                          <span className="detail-label">Aircraft:</span>
                          <span className="detail-value">{segment.flightDetails.aircraft}</span>
                        </div>
                      </div>
                    )}
                    ...
                    {segment.mode === 'Sea' && segment.vesselDetails && (
                      <div className="additional-details">
                        ...
                        <div className="detail-item">
                          ...
                          <span className="detail-value">{segment.vesselDetails.capacity}</span>
                        </div>
                        <div className="detail-item">
                          <span className="detail-label">Route:</span>
                          <span className="detail-value">{segment.vesselDetails.route}</span>
                        </div>
                      </div>
                    )}
                    ...
                    {segment.mode === 'Land' && segment.trainDetails && (
                      <div className="additional-details">
                        ...
                        <div className="detail-item">
                          ...
                          <span className="detail-value">{segment.trainDetails.trainNumber}</span>
                        </div>
                        <div className="detail-item">
                          <span className="detail-label">Carriage:</span>
                          <span className="detail-value">{segment.trainDetails.carriageNumber}</span>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="results-container">
          ...
          <p className="route-summary">From <strong>{shipmentDetails.origin}</strong> to <strong>{shipmentDetails.destination}</strong></p>
            ...
            <div className="route-result-section">
              ...
              <div className="route-options">
                {getFeaturedRoutes().map((route) => (
                  <div key={route.id} className={`route-card featured featured-${route.category}`}>
                    <div className="route-header">
                      <h3>
                        {route.category === 'speed' ? 'Fastest Option' : 
                          route.category === 'cost' ? 'Most Cost-Effective' : 
                          `Option ${route.id}`}
                      </h3>
                      <div className="route-modes">
                        {route.modes.map((mode, index) => (
                          <span key={index} className={`mode-tag mode-${mode.toLowerCase()}`}>
                            {mode}
                            {index < route.modes.length - 1 && <span className="mode-arrow">→</span>}
                          </span>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="route-details">
                      ...
                      <div className="detail-item">
                        ...
                        <span className="detail-value">{route.transitTime} days</span>
                      </div>
                      <div className="detail-item">
                        <span className="detail-label">Border Crossings:</span>
                        <span className="detail-value">{route.borderCrossings}</span>
                      </div>
                      <div className="detail-item">
          ...
          {getFilteredRoutes().length === 0 && (
            <div className="no-results">
              <p>No routes found for the selected filter. Try a different option.</p>
            </div>
          )}
